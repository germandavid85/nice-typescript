...
export class Rule extends Lint.Rules.AbstractRule {
  public static FAILURE_STRING = 'single responsibility check';

  public apply(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
    return this.applyWithWalker(new NoSingleResponsibilityWalker(sourceFile, this.ruleName, new Set(this.ruleArguments.map(String))));
  }
}

class NoSingleResponsibilityWalker extends Lint.AbstractWalker<Set<string>> {
  public walk(sourceFile: ts.SourceFile) {
    if (sourceFile.fileName.endsWith('single.ts')){
